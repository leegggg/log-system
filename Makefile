...
WIKI=$(shell find -maxdepth 1 -name "*.md" | grep -E [0-9]+.* ) # 从本路径向下查找所有.mkd作为源文件
HTML=$(WIKI:%.md=html/%.html) # 将.mkd的同名.html文件作为目标文件

# pandoc选项
#PANDOC_FLAG=  --toc # 自动生成目录
PANDOC_FLAG+= --css="res/template/style.css" # 指名css样式文件.
PANDOC_FLAG+= --template="res/template/pandoctpl.html" # pandoc模板.
