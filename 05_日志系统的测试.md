...
如不经过Kafka请将Line 56 改为name: logstash-pipeline-direct-config
* Logstash for app log files
    kubectl --namespace=elk create -f logstash-app/logstash-deployment-app.yaml
如不经过Kafka请将Line 57 改为name: logstash-app-direct-pipeline-config
* Logstash shipper（仅供使用kafka时使用） 
用于从kafka中向ES搬运数据
