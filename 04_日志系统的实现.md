...
这个头部指明了，这个请求通过JWT鉴权，签名算法是HS256算法。JWT一般可以选择使用非对称的RSA或者对称的HMAC算法对token签名。MSCP系统选择使用HMAC算法来进行签名。这是一种对称加密，加密操作和解密操作使用同一个密钥，其优势在于计算量比较小。缺点在于无法支持第三方签发token的场景。但是MSCP系统上的token都由MSCP API自己签发，所以HMAC算法即可以满足需求。这个头部经过Base64编码，成为jwt的第二部分，下文称编码2。
...
!["mscp orth log setting"](mscp orth_log_.png )
