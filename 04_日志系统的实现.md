...
MSCP系统在签发token时会使用来自数据库中的一组密钥，通过HMAC-SHA256算法来对上文中编码1和编码2用句号`.`连接在一起（头部在前）形成的形如`编码2.编码1`的字符串通过加密和哈希来形成一个签名，下文称编码3。此密钥会经常更改以降低泄露风险。
最后将这一部分签名也拼接在被签名的字符串后面，我们就得到了完整的JWT，形如`编码2.编码1。编码3`
这一组信息会在在HTTP头部的`Authorization`字段或者直接在请求URL的`jwt`参数中被传输。API Server在收到请求后会通过验证签名来验证token的有效性。
